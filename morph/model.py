...
class MORPH_no_residual1(nn.Module):
    '''
    Variant of MORPH — residual connection removed from the first attention module
    '''
    def __init__(self, dim, c_dim, 
                 opts,
                 device=None):
        
        super(MORPH_no_residual1, self).__init__()
...
class MORPH_moe_3expert(nn.Module):
    '''
    Variant of MORPH -- combine the three strongest priors
    '''
    def __init__(self, dim, c_dim, 
                 opts,
                 device=None):
        
        super(MORPH_moe_3expert, self).__init__()
        ...
        self.z_dim_ptb = opts.latdim_ptb #latent dimension for perturbation decoder
        self.c_dim = c_dim #input dimension for perturbation decoder
        self.c_dim_2 = opts.cdim_2
        self.c_dim_3 = opts.cdim_3
        self.dim = dim #input dimension for gene-expression decoder
        self.geneset_num = opts.geneset_num # specify number of geneset in the geneset matrix
        self.geneset_dim = opts.geneset_dim # specify the dimension of the gene set matrix
        self.hidden_dim = opts.hidden_dim
        ...
        if opts.null_label == 'zeros':
            self.null_label_1 = torch.zeros(1, self.c_dim).double().to(self.device)
            self.null_label_2 = torch.zeros(1, self.c_dim_2).double().to(self.device)
            self.null_label_3 = torch.zeros(1, self.c_dim_3).double().to(self.device)
        elif opts.null_label == 'gaussian':
            null_folder = f'{opts.base_dir}/data/gene_ptb_emb/Null/dim_{self.c_dim}/null_vector.npy'
            self.null_label_1 = torch.tensor(np.load(null_folder)).double().to(self.device)
            null_folder = f'{opts.base_dir}/data/gene_ptb_emb/Null/dim_{self.c_dim_2}/null_vector.npy'
            self.null_label_2 = torch.tensor(np.load(null_folder)).double().to(self.device)
            null_folder = f'{opts.base_dir}/data/gene_ptb_emb/Null/dim_{self.c_dim_3}/null_vector.npy'
            self.null_label_3 = torch.tensor(np.load(null_folder)).double().to(self.device)
        elif opts.null_label == 'gaussian_normalized':
            null_folder = f'{opts.base_dir}/data/gene_ptb_emb/Null/dim_{self.c_dim}/normalized_vector.npy'
            self.null_label_1 = torch.tensor(np.load(null_folder)).double().to(self.device)
            null_folder = f'{opts.base_dir}/data/gene_ptb_emb/Null/dim_{self.c_dim_2}/normalized_vector.npy'
            self.null_label_2 = torch.tensor(np.load(null_folder)).double().to(self.device)
            null_folder = f'{opts.base_dir}/data/gene_ptb_emb/Null/dim_{self.c_dim_3}/normalized_vector.npy'
            self.null_label_3 = torch.tensor(np.load(null_folder)).double().to(self.device)
        ...
        # Perturbation encoder (expert 1)
        self.expert_1 = nn.Sequential(
            nn.Linear(self.c_dim, self.hidden_dim), #c_dim -> hids
            nn.LeakyReLU(0.2),
            nn.Linear(self.hidden_dim, self.z_dim_ptb), #c_dim -> hids
            nn.LeakyReLU(0.2)
        )

        # Perturbation encoder (expert 2)
        self.expert_2 = nn.Sequential(
            nn.Linear(self.c_dim_2, self.hidden_dim), #c_dim -> hids
            nn.LeakyReLU(0.2),
            nn.Linear(self.hidden_dim, self.z_dim_ptb), #c_dim -> hids
            nn.LeakyReLU(0.2)
        )

        # Perturbation encoder (expert 3)
        self.expert_3 = nn.Sequential(
            nn.Linear(self.c_dim_3, self.hidden_dim), #c_dim -> hids
            nn.LeakyReLU(0.2),
            nn.Linear(self.hidden_dim, self.z_dim_ptb), #c_dim -> hids
            nn.LeakyReLU(0.2)
        )
        ...
        self.gate = nn.Sequential(
            nn.Linear(self.z_dim_ptb*3, self.z_dim_ptb),
            nn.Sigmoid()
        )
    ...
    def forward(self, x, c_1, c_2, c_1_2, c_2_2, c_1_3, c_2_3, gene_set_mtrx = None, return_alphas = False, return_latents = False):

        # Output from expert 1
        z_ptb_1 = self.expert_1(c_1)
        z_ptb_2 = None
        c_2_test = c_2.detach().cpu().numpy()
        if not np.all(np.isnan(c_2_test)):
            z_ptb_2 = self.expert_1(c_2)
            del c_2_test
        
        # Output from expert 2
        z_ptb_1_2 = self.expert_2(c_1_2)
        z_ptb_2_2 = None
        c_2_2_test = c_2_2.detach().cpu().numpy()
        if not np.all(np.isnan(c_2_2_test)):
            z_ptb_2_2 = self.expert_2(c_2_2)
            del c_2_2_test

        # Output from expert 3
        z_ptb_1_3 = self.expert_3(c_1_3)
        z_ptb_2_3 = None
        c_2_3_test = c_2_3.detach().cpu().numpy()
        if not np.all(np.isnan(c_2_3_test)):
            z_ptb_2_3 = self.expert_3(c_2_3)
            del c_2_3_test

        # Now, input to the Gating network
        if z_ptb_2 is None:
            z_ptb_ft = self.gate(torch.cat((z_ptb_1, z_ptb_1_2, z_ptb_1_3), dim=1))
        else:
            z_ptb_a = self.gate(torch.cat((z_ptb_1, z_ptb_1_2, z_ptb_1_3), dim=1))
            z_ptb_b = self.gate(torch.cat((z_ptb_2, z_ptb_2_2, z_ptb_2_3), dim=1))
            z_ptb_ft = z_ptb_a + z_ptb_b

        # encode control label
        control_label = self.null_label_1.repeat(c_1.size(0), 1)
        z_control_a = self.expert_1(control_label)
        control_label = self.null_label_2.repeat(c_1_2.size(0), 1)
        z_control_b = self.expert_2(control_label)
        control_label = self.null_label_3.repeat(c_1_3.size(0), 1)
        z_control_c = self.expert_3(control_label)
        z_control = self.gate(torch.cat((z_control_a, z_control_b, z_control_c), dim=1))
...
class MORPH_no_attention(nn.Module):
    '''
    Variant of MORPH without attention mechanisms - ablation study model
    '''
    def __init__(self, dim, c_dim, 
                 opts,
                 device=None):
        
        super(MORPH_no_attention, self).__init__()
        ...
        self.c_dim = c_dim #input dimension for perturbation decoder
        self.dim = dim #input dimension for gene-expression decoder
        # Use hidden_dim_ablation if available, otherwise fall back to hidden_dim
        self.hidden_dim = getattr(opts, 'hidden_dim_ablation', opts.hidden_dim)
        ...
        # Decoder MLP
        self.d1 = nn.Linear(self.z_dim_gene + self.z_dim_ptb, self.hidden_dim) #z_dim -> hidden_dim
        self.d2 = nn.Linear(self.hidden_dim, self.dim) #hidden_dim -> dim
    ...
    def forward(self, x, c_1, c_2, gene_set_mtrx = None, return_latents = False):
